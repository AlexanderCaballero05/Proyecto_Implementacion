...
// --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
			...
			//volvemos a definir el  encabezado cuando se crea una nueva pagin	
			$this->SetFont('Helvetica', 'B', 10);
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="select CONCAT_WS(' ',p.PRIMER_NOMBRE,p.SEGUNDO_NOMBRE,p.PRIMER_APELLIDO,p.SEGUNDO_APELLIDO) as nombre  ,p.DNI ,t.NUMERO_TELEFONO as telefono,
    p.FECHA_NACIMIENTO,s.SEXO ,c.CORREO_PERSONA
     from tbl_expediente_medico ex, tbl_persona p, tbl_telefono t ,tbl_sexo s,tbl_correo_electronico c
...
$pdf->SetWidths(array(15,65,30,35,26,22,50)); //cambiar
...
for ($i = 0; $i < count($data); $i++) {
    ...
    ,ucwords(strtolower(utf8_decode($data[$i]['SEXO']))) ,ucwords(strtolower(utf8_decode($data[$i]['telefono']))) 
 ,ucwords(strtolower(utf8_decode($data[$i]['CORREO_PERSONA']))) ,),25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
