...
for ($i = 0; $i < count($data); $i++) {
	...
	$pdf->Row(array($i + 1, $data[$i]['FAMILIAR'], $data[$i]['ESTADO_CIVIL'],ucwords(strtolower(utf8_decode($data[$i]['NIVEL_EDUCATIVO']))), ucwords(strtolower(utf8_decode($data[$i]['INGRESOS_DE_FAMILIAR']))),ucwords(strtolower(utf8_decode($data[$i]['NOMBRE_IGLESIA']))),  ),25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
