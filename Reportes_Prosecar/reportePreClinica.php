...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(120);
		$this->Cell(180, 12, utf8_decode('Reporte de Pre-Clinica'));
		$this->SetX(5);
		$this->Ln(5);
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="SELECT p.CODIGO_PRECLINICA , CONCAT_WS(' ',pe.PRIMER_NOMBRE,pe.SEGUNDO_NOMBRE,pe.PRIMER_APELLIDO,pe.SEGUNDO_APELLIDO) 
...
$pdf->Cell(17, 12, 'Sangre', 1, 0, 'C', 1);
$pdf->Cell(17, 12, 'Peso', 1, 0, 'C', 1);
$pdf->Cell(22, 12, 'Estatura', 1, 0, 'C', 1);
$pdf->Cell(70, 12, 'Alergias', 1, 1, 'C', 1);
...
$pdf->SetWidths(array(10,50, 17, 17, 22,70)); //???

for ($i = 0; $i < count($data); $i++) {

	$pdf->Row(array($i + 1,$data[$i]['NOMBRE_PACIENTE'], ucwords(strtolower(utf8_decode($data[$i]['TIPO_SANGRE']))),$data[$i]['PESO'], $data[$i]['ESTATURA'], $data[$i]['ALERGIAS'] ),50 ); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
