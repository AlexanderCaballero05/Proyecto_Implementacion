...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(73);
		$this->Cell(45, 12, utf8_decode('Reporte de lista de  Tutorias '));
		$this->SetX(5);
		$this->Ln(11);
...
$pdf->Cell(50, 12, 'NOMBRE TUTORIA', 1, 0, 'C', 1);
$pdf->Cell(75, 12, 'AREA', 1, 1, 'C', 1);
...
for ($i = 0; $i < count($data); $i++) {
	$pdf->Row(array($i + 1, ucwords((utf8_decode($data[$i]['TUTORIA']))) ,ucwords((utf8_decode($data[$i]['AREA']))) ),30); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
