...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(120);
		$this->Cell(180, 8, utf8_decode('Reporte de receta medica'));
		$this->SetX(5);
		$this->Ln(5);
...
// --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
			...
			$this->Cell(70, 8, 'Nombre paciente', 1, 0, 'C', 0);
			$this->Cell(50, 8, 'Nombre medicamento', 1, 0, 'C', 0);
			$this->Cell(50, 8, 'Indicaciones receta', 1, 0, 'C', 0);
			$this->Cell(50, 8, 'Observaciones', 1, 0, 'C', 0);
            $this->Cell(30, 8, 'Fecha receta', 1, 0, 'C', 0);
			$this->SetFont('Arial', '', 10);
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="SELECT CONCAT_WS (' ','DNI',DNI,PRIMER_NOMBRE, ' ',SEGUNDO_NOMBRE,' ',PRIMER_APELLIDO)
...
$pdf->Cell(70, 12, 'Nombre paciente', 1, 0, 'C', 1);
$pdf->Cell(50, 12, 'Nombre medicamento', 1, 0, 'C', 1);
$pdf->Cell(50, 12, 'Indicaciones receta', 1, 0, 'C', 1);
$pdf->Cell(50, 12, 'Observaciones', 1, 0, 'C', 1);
$pdf->Cell(30, 12, 'Fecha receta', 1, 1, 'C', 1);
...
$pdf->SetWidths(array(15, 70, 50, 50,50,30 )); //???

for ($i = 0; $i < count($data); $i++) {

    $pdf->Row(array($i + 1, $data[$i]['NOMBRE'], $data[$i]['NOMBRE_MEDICAMENTO'],ucwords(strtolower(utf8_decode($data[$i]['INDICACIONES_RECETA']))), ucwords(strtolower(utf8_decode($data[$i]['OBSERVACIONES']))),ucwords(strtolower(utf8_decode($data[$i]['FECHA_RECETA']))), ),20); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
