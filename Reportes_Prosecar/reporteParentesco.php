...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(73);
		$this->Cell(45, 13, utf8_decode('Reporte de Parentesco'));
		$this->Ln(8);
		$this->SetX(5);
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="SELECT CODIGO_PARENTESCO, NOMBRE AS PARENTESCO
    FROM TBL_PARENTESCO; ";
...
$pdf->Cell(75, 12, 'Parentesco', 1, 1, 'C', 1);
...
for ($i = 0; $i < count($data); $i++) {
	$pdf->Row(array($data[$i]['CODIGO_PARENTESCO'], ucwords(strtolower(utf8_decode($data[$i]['PARENTESCO'])))  ),60); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
