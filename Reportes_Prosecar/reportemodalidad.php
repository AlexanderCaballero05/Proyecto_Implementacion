...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(80);
		$this->Cell(45, 12, utf8_decode('Reporte de Modalidad'));
	    $this->Ln(8);
		$this->SetX(5);
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="SELECT CODIGO_MODALIDA, TIPO, DESCRIPCION from tbl_modalidad ;";
...
$pdf->SetWidths(array(10,35,110)); //???

for ($i = 0; $i < count($data); $i++) {
	$pdf->Row(array($i + 1, ucwords(strtolower(utf8_decode($data[$i]['TIPO']))), ucwords(strtolower(utf8_decode($data[$i]['DESCRIPCION']))),  ),25); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA//EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
