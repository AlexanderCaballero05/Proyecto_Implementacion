...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(73);
		$this->Cell(45, 12, utf8_decode('Reporte de Tipo Socioeconomico'));
		$this->SetX(5);
		$this->Ln(11);
...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="SELECT TIPO,DESCRIPCION from tbl_tipo_socioeconomico";
...
for ($i = 0; $i < count($data); $i++) {
	$pdf->Row(array($i + 1, ucwords(strtolower(utf8_decode($data[$i]['TIPO']))), ucwords(strtolower(utf8_decode($data[$i]['DESCRIPCION']))),  ),20); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA//EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
