...
// Cabecera de página
	...
	function Header() {
		...
		$this->SetX(125);
		$this->Cell(140, 12, utf8_decode('Reporte de Estados'));
		$this->SetX(5);
		$this->Ln(5);
...
// --------------------METODO PARA ADAPTAR LAS CELDAS------------------------------
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
            ...
            $pdf->Cell(130, 12, 'Descripcion', 1, 1, 'C', 1);
...
$pdf->SetWidths(array(10, 50,130)); //???

for ($i = 0; $i < count($data); $i++) {

	$pdf->Row(array($i + 1 ,ucwords(strtolower(utf8_decode($data[$i]['NOMBRE']))),  ucwords(strtolower(utf8_decode($data[$i]['DESCRIPCION'])))  ),50 ); 
