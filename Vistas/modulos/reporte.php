...
//------------------OBTENES LOS DATOS DE LA BASE DE DATOS-------------------------
  ...
  $conexion=$data->conect(); 
	$strquery ="SELECT c.CODIGO_CARGA,t.NOMBRE as TUTORIA,  CONCAT_WS(' ',p.PRIMER_NOMBRE,p.SEGUNDO_NOMBRE,p.PRIMER_APELLIDO,p.SEGUNDO_APELLIDO) 
...
$pdf->SetWidths(array(30, 30, 40, 55,25,30,30)); //???

for ($i = 0; $i < count($data); $i++) {

	$pdf->Row(array($data[$i]['SECCION'], ucwords(strtolower(utf8_decode($data[$i]['MODALIDAD']))),ucwords(strtolower(utf8_decode($data[$i]['TUTORIA']))), ucwords(strtolower(utf8_decode($data[$i]['NOMBRE_COMPLETO']))), utf8_decode($data[$i]['HORA']), $data[$i]['FECHA_INICIO'], $data[$i]['FECHA_FINAL']   ),28 ); //EL 28 ES EL MARGEN QUE TIENE DE DERECHA
}
